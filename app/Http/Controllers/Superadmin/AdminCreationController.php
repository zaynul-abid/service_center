...
class AdminCreationController extends Controller
{
    ...
    public function edit(User $superadmin_admin)
    {
        ...
        return view('superadmin.admin.edit', compact('superadmin_admin'));
    }

    public function update(Request $request, User $superadmin_admin)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'email' => ['required', 'email', 'max:255', Rule::unique('users')->ignore($superadmin_admin->id)],
